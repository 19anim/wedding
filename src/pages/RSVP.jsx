...
const RSVP = () => {
  ...
  return (
    <section id="rsvp" className="snap-start w-full h-screen bg-[#e1e1e1] flex items-center justify-center flex-col lg:text-base text-xs relative">
      <section className={`bg-[rgba(0,0,0,0.2)] w-full h-full absolute left-0 z-50 ${!isSubmit ? "hidden" : null}`}></section>
      <section className="text-black w-[90%] h-[55%] lg:h-[70%] min-h-[500px] lg:min-h-[600px] lg:w-[45%] bg-white text-center relative flex justify-center shadow-[0_10px_40px_rgba(0,0,0,0.3),0_4px_10px_rgba(0,0,0,0.2),inset_0_2px_5px_rgba(255,255,255,0.5)] rounded-3xl max-w-[550px]">
        <header className="lg:tracking-[1rem] font-greatVibes text-5xl lg:text-6xl absolute top-[10%] mx-auto w-full">R.S.V.P</header>
        ...
        <section className="absolute top-[20%] right-[5%] text-right font-montserrat">
          ...
          <header className="font-bold text-base tracking-[0.2rem] lg:text-lg lg:tracking-[0.5rem]">MARCH 01, 2025</header>
        </section>
        ...
        <form onSubmit={onSubmitHandler} className="absolute font-montserrat top-[43%] w-full mt-5 flex flex-col items-center">
          <section className="font-montserrat w-[80%] flex gap-5">
            <label htmlFor="customerName">Mr/Ms</label>
            <input onChange={onChangeHandler} name="customerName" type="text" className="outline-none border-b border-black grow text-center" />
          </section>
          ...
          <section className="pt-5 grid grid-cols-2 gap-y-2">
            <section className="font-montserrat w-full m-auto font-semibold col-span-2">ARE YOU VEGETARIAN ?</section>
            <label>
